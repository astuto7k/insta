<template>
  <div
    class="min-h-screen bg-black text-white font-sans relative overflow-hidden flex flex-col"
    style="background-color: #000; min-height: 100vh; display: flex; flex-direction: column; overflow: hidden; position: relative;"
  >
    <!-- Matrix Background -->
    <canvas
      ref="matrixCanvas"
      class="fixed inset-0 w-full h-full opacity-20 pointer-events-none z-0"
      style="position: fixed; inset: 0; width: 100%; height: 100%; opacity: 0.2; pointer-events: none; z-index: 0;"
    />

    <!-- Content Wrapper -->
    <div
      class="relative z-10 flex-1 flex flex-col items-center pb-24 overflow-y-auto w-full max-w-[450px] mx-auto border-x border-gray-800/50 bg-black/50 backdrop-blur-sm"
      style="position: relative; z-index: 10; flex: 1; display: flex; flex-direction: column; align-items: center; width: 100%; max-width: 450px; margin-left: auto; margin-right: auto; background-color: rgba(0,0,0,0.5);"
    >
      <!-- Logo Area -->
      <div class="stomp-animation stomp-delay-1 pt-8 pb-2 flex flex-col items-center gap-2">
        <div class="flex items-center gap-2">
          ...
          />
          <span class="text-xl font-bold tracking-widest text-white">INSTA <span class="text-purple-500">ESPIÃO</span></span>
        </div>
        ...
        <div class="bg-[#1C1C1E] border border-gray-800 p-5 rounded-3xl flex items-center gap-4 shadow-xl">
          <div class="p-[2px] bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-500 rounded-full">
            <div class="bg-black p-[2px] rounded-full">
              ...
              <div
                v-else
                class="w-16 h-16 rounded-full bg-gray-700 animate-pulse"
              />
            </div>
          </div>
      ...
      <div class="w-full px-6 mt-6 stomp-animation stomp-delay-3">
        <div class="bg-[#1a0b2e] border border-purple-500/30 rounded-2xl p-4 text-center shadow-[0_0_20px_rgba(168,85,247,0.15)] relative overflow-hidden">
          <!-- Shine effect -->
          <div class="absolute top-0 left-0 w-full h-full bg-gradient-to-r from-transparent via-purple-500/10 to-transparent -translate-x-full animate-shine" />

          <p class="font-bold text-white mb-1">
            Espionagem 100% finalizada! 🥳
          </p>
      ...
      <div class="w-full px-6 mb-8 stomp-animation stomp-delay-5">
        <div class="flex items-center gap-3 mb-4">
          ...
          <h2 class="font-bold text-lg">
            Veja Mídias de {{ userProfile?.username || 'Usuário' }}
          </h2>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div
            v-for="i in 4"
            :key="i"
            class="aspect-square bg-zinc-900 rounded-2xl overflow-hidden relative border border-gray-800"
          >
            <!-- Fake blurred content -->
            <img
              :src="getProxyUrl(fakeImages[(i-1) % fakeImages.length] || '')"
      ...
      <div class="w-full px-6 mb-8 stomp-animation stomp-delay-6">
        ...
        <div class="bg-zinc-900 rounded-2xl overflow-hidden border border-gray-800 h-48 relative">
          ...
          <div class="w-full h-full bg-[#242f3e]  relative">
            <img
              :src="getProxyUrl('/location/fake-location-bg.png')"
          ...
          <div class="absolute bottom-4 left-4 right-4 p-3 rounded-xl flex flex-col items-center">
            ...
            <button
              class="w-full mt-3 bg-[#2C2C2E] py-2 rounded-lg text-xs font-bold hover:bg-zinc-700 text-white border border-gray-600"
      ...
      <div class="w-full px-6 mb-8 stomp-animation stomp-delay-7">
        ...
        <div class="flex gap-3 justify-center">
          <div
            v-for="i in 2"
            :key="`story-${i}`"
            class="w-[140px] h-[220px] bg-zinc-900 rounded-2xl border border-gray-800 relative overflow-hidden"
          >
            <img
              :src="getProxyUrl(fakeImages[i % fakeImages.length] || '')"
            ...
            <div
              v-if="i===2"
              class="absolute top-3 right-3 w-3 h-3 bg-green-500 rounded-full shadow-[0_0_10px_#22c55e]"
            />
          </div>
        </div>
      </div>
          ...
          <div class="flex justify-between items-center border-b border-gray-800 pb-3 mb-3">
            <div class="flex items-center gap-3">
              <div class="relative">
                ...
                />
                <div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-[#1C1C1E] rounded-full" />
              </div>
              <div class="flex flex-col">
          ...
          <div class="flex flex-col gap-3">
            <div class="bg-zinc-800 self-start rounded-2xl rounded-tl-none px-4 py-2 max-w-[85%]">
              <p class="text-gray-300 text-sm">
                E aí, bora ver tudo do instagram de {{ userProfile?.username || '...' }}?
              </p>
            </div>

            <!-- CTA Button inside chat -->
            <button
              class="bg-gradient-to-r from-purple-600 to-indigo-600 self-end rounded-full px-6 py-3 mt-2 shadow-lg hover:scale-105 transition-transform flex items-center gap-2"
              @click="showBlockedModal = true"
            >
      ...
      <div
        id="pricing-section"
        class="my-6 animate-bounce stomp-animation stomp-delay-9"
      >
        <UIcon
          name="i-lucide-arrow-down"
          class="w-8 h-8 text-white"
      ...
      <div class="w-full px-6 mb-8 stomp-animation stomp-delay-10">
        <div class="bg-[#0f0f11] border border-gray-800/80 rounded-[32px] p-6 shadow-2xl relative overflow-hidden">
          <div class="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-purple-500/50 to-transparent" />
          ...
          <div class="flex flex-col items-center text-center mb-6">
            <div class="flex items-center gap-1 mb-3">
              <UIcon
                ...
                class="w-5 h-5 text-purple-500"
              />
              <span class="font-bold text-white tracking-widest">INSTA <span class="text-purple-500">ESPIÃO</span></span>
            </div>
            <h2 class="text-xl font-bold leading-tight">
              Com o <span class="text-purple-500">Insta Espião</span> você vai ter<br>acesso completo ao instagram<br>de {{ userProfile?.username || username }} por apenas:
            </h2>
          </div>
      ...
      <div class="w-full px-6 mb-8 stomp-animation stomp-delay-11">
        <button
          class="w-full bg-gradient-to-br from-[#8b5cf6] to-[#6d28d9] rounded-2xl p-1 shadow-[0_0_20px_rgba(139,92,246,0.5)] group transform transition hover:scale-[1.02]"
          style="width: 100%; cursor: pointer;"
          @click="goToCheckout"
        >
          <div
      ...
      <div class="w-full px-6 mb-8 stomp-animation stomp-delay-13">
        <h3 class="text-center font-bold text-lg mb-1">
          Veja o que falam as pessoas
        </h3>
        <p class="text-center text-gray-400 text-sm mb-6">
          que usam o <span class="text-purple-500 font-bold">Insta Espião</span>
        </p>
        ...
        <!-- Screenshot Carousel -->
        <div class="mt-8">
          <h4 class="text-center font-bold text-md text-white mb-4">
            Outros depoimentos
          </h4>
          <UCarousel
            v-slot="{ item }"
          ...
          >
            <div class="p-1">
              <img
                ...
                class="w-full rounded-xl border border-gray-800 object-cover"
                draggable="false"
              >
            </div>
          </UCarousel>
      ...
      <div class="w-full px-6 mb-8 stomp-animation stomp-delay-15">
        <h3 class="text-center font-bold text-lg mb-6">
    ...
    <!-- Fixed Bottom Bar -->
    <div
      class="slide-up-animation fixed bottom-0 left-0 right-0 bg-[#151517]/95 backdrop-blur-md border-t border-gray-800 p-4 pb-6 z-50"
      style="position: fixed; bottom: 0; left: 0; right: 0; z-index: 50; background-color: rgba(21, 21, 23, 0.95);"
    >
      <div class="max-w-[450px] mx-auto flex items-center justify-between gap-4">
        ...
        <button
          class="bg-[#584cea] hover:bg-[#4a3fcb] text-white font-bold text-xs px-4 py-3 rounded-xl shadow-lg whitespace-nowrap leading-tight"
          @click="goToCheckout"
        >
    ...
    >
      <div class="bg-[#1C1C1E] border border-gray-800 w-full max-w-[320px] rounded-2xl p-6 flex flex-col items-center text-center shadow-2xl relative">
        ...
        <button
          class="w-full bg-gradient-to-r from-[#7C4DFF] to-[#9A6CFF] text-white font-semibold py-3 rounded-xl transition-colors hover:opacity-90 mb-3"
          @click="goToCheckout"
        >
          Adquirir Plano VIP
        </button>
...
const faqItems = ref([
  {
    ...
    answer: 'Sim! O Insta Espião utiliza tecnologia avançada para acessar dados do Instagram de forma completamente invisível. Milhares de usuários já confirmaram a eficácia da ferramenta.',
    open: true
  },
  {
  ...
  },
  {
    ...
    answer: 'Não! O Insta Espião funciona diretamente no seu navegador. Não é necessário baixar nenhum aplicativo ou software.',
    open: false
  },
  {
