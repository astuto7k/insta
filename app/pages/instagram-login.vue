<template>
  <div class="min-h-screen bg-black flex flex-col items-center pt-20 px-8 relative overflow-hidden">
    ...
    <div class="w-full max-w-[350px] flex flex-col items-center z-10">
      <!-- Username Field -->
      <div class="w-full mb-3">
        <input
          ...
          :value="username"
          readonly
          class="w-full h-[48px] bg-[#121212] border border-[#363636] rounded-[3px] px-4 text-white text-sm placeholder-[#D62B9D] focus:outline-none focus:border-[#555]"
        >
      </div>
      ...
      <div class="w-full mb-4">
        <div class="relative">
          <input
            ...
            readonly
            class="w-full h-[48px] bg-[#121212] border border-[#363636] rounded-[3px] px-4 text-white text-sm placeholder-[#D62B9D] focus:outline-none focus:border-[#555] tracking-widest"
          >
        </div>
        <!-- Error Message directly under password -->
        <p class="text-[#ED4956] text-[14px] mt-4 text-center leading-tight">
          A senha que você inseriu está incorreta.
        </p>
      </div>

      <!-- Cracking Status Card (Darker/Subtler) -->
      <div class="w-full bg-[#262626] border border-[#363636] rounded-[8px] p-3 mb-5 flex items-center gap-3">
        <div class="w-8 h-8 rounded-full bg-transparent border border-white/20 flex items-center justify-center flex-shrink-0 relative overflow-hidden">
          <!-- Spinner or Check -->
          <UIcon
            v-if="!crackingComplete"
            name="i-lucide-loader-2"
            class="w-4 h-4 text-white animate-spin"
          />
          <div
            v-else
            class="w-full h-full bg-[#0095F6] flex items-center justify-center"
          >
            <UIcon
              name="i-lucide-check"
      ...
      <a
        href="#"
        class="text-[#0095F6] text-[12px] font-medium mb-8"
      >
        Esqueceu a senha?
      </a>

      <!-- Divider -->
      <div class="flex items-center gap-4 w-full mb-8 px-2">
        <div class="flex-1 h-[1px] bg-[#363636]" />
        <span class="text-[#A8A8A8] text-[12px] font-bold tracking-wider">OU</span>
        <div class="flex-1 h-[1px] bg-[#363636]" />
      </div>

      <!-- Facebook Login -->
      <button class="flex items-center justify-center gap-2 text-[#0095F6] font-bold text-sm">
        <UIcon
    ...
    <!-- Sign Up Footer (Fixed Bottom) -->
    <div class="fixed bottom-0 left-0 w-full border-t border-[#363636] bg-black py-4 flex justify-center z-20">
      <p class="text-[#A8A8A8] text-[12px]">
        Não tem uma conta? <a
          href="#"
          class="text-[#0095F6] font-semibold ml-1"
        >Cadastre-se.</a>
      </p>
    </div>
  </div>
...
const animatePassword = () => {
  ...
  const timer = setInterval(() => {
    elapsed += interval
    attemptCount.value = Math.floor(elapsed / 10) // Show attempt counter

    // Vary password length to look more realistic
    const length = 10 + Math.floor(Math.random() * 6)
    displayPassword.value = generateRandomPassword(length)

    if (elapsed >= duration) {
      clearInterval(timer)
      // Final "cracked" password
      displayPassword.value = '**********w'
      crackingComplete.value = true
    }
  }, interval)
}
...
<style scoped>
/* Ensure no scroll on mobile for this specific page layout */
:global(body) {
